// Frogger enhanced edition
// added feature: Set different colours for each lines of the blocks
...
void initX(int argc, char* argv[], XInfo& xinfo) {
    ...
    rc = XAllocNamedColor(xinfo.display, cmap, "red", &red, &red);
    if (rc == 0) {
        cerr<<"XAllocNamedColor - failed to allocated 'red' color.\n";
        exit(1);
    }
    rc = XAllocNamedColor(xinfo.display, cmap, "brown", &brown, &brown);
    if (rc == 0) {
        cerr<<"XAllocNamedColor - failed to allocated 'brown' color.\n";
        exit(1);
    }
    rc = XAllocNamedColor(xinfo.display, cmap, "blue", &blue, &blue);
    if (rc == 0) {
        cerr<<"XAllocNamedColor - failed to allocated 'blue' color.\n";
        exit(1);
    }
    rc = XAllocNamedColor(xinfo.display, cmap, "yellow", &yellow, &yellow);
    if (rc == 0) {
        cerr<<"XAllocNamedColor - failed to allocated 'yellow' color.\n";
        exit(1);
    }
    rc = XAllocNamedColor(xinfo.display, cmap, "green", &green, &green);
    if (rc == 0) {
        cerr<<"XAllocNamedColor - failed to allocated 'green' color.\n";
        exit(1);
    }
